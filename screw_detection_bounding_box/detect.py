...
model = YOLO('runs/detect/train/weights/best.pt').to(device=0)
x_crosshair = 500
y_crosshair = 512
...
def run_model(img):
    results = model(img, save=True, imgsz=640, conf=0.7, half=False)
...
def draw_angle_label(img, angle, position):
    ...
    draw.text(position, text=text, font=font, fill=font_color)
